...
export default function SingleBookTocPage() {
  ...
  const [role, setRole] =
    useState<"viewer" | "author" | "editor" | null>(null);
  ...
  async function loadItem(tocItemId: string) {
    try {
      ...
      const c = json.content?.content_json ?? {
        type: "doc",
        content: [],
      };
  ...
  // Resolve version + load tree khi auth ready
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!vId) {
          setErrorMsg(
            "Sách này chưa có version (book_versions). Hãy tạo version trước."
          );
          setVersionId("");
          setLoading(false);
          return;
  ...
  async function onCreateItem(parent_id: string | null) {
    ...
    if (!title) return;
    if (!versionId) return alert("Thiếu versionId");
    ...
    if (!res.ok) return alert(json?.error || "Tạo mục thất bại");
  ...
  async function handleDropOnItem(e: React.DragEvent, target: TocItem) {
    ...
    const fromSibs = (byParent.get(fromParent) || []).filter(
      (x) => x.id !== dragId
    );
    ...
    if (e.ctrlKey) {
      const appended = [
        ...toSibs.filter((x) => x.id !== dragId),
        next[draggedIdx],
      ];
      appended.forEach((x, idx) => (x.order_index = idx + 1));
      fromSibs.forEach((x, idx) => (x.order_index = idx + 1));
      setItems(
        byParentToFlat(byParent, fromParent, fromSibs, toParent, appended)
      );
    } else {
      const targetParent = target.parent_id ?? null;
      const sibs = (byParent.get(targetParent) || []).filter(
        (x) => x.id !== dragId
      );
      ...
      if (fromParent !== targetParent) {
        fromSibs.forEach((x, idx) => (x.order_index = idx + 1));
        setItems(
          byParentToFlat(byParent, fromParent, fromSibs, targetParent, sibs)
        );
      } else {
        setItems(next);
      }
  ...
  async function onRemoveAssignment(user_id: string) {
    ...
    const { res, json } = await fetchJsonWithTimeout(
      `/api/toc/assignments?toc_item_id=${encodeURIComponent(
        selectedId
  ...
  if (authLoading) return <div className="p-6">Đang xác thực...</div>;
  if (!user) return null;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-2xl p-3">
          ...
          <div className="space-y-1">
            ...
            {!roots.length && (
              <div className="text-gray-600">Chưa có mục nào.</div>
            )}
          </div>

          {canEditToc && (
            <div className="mt-3 text-xs text-gray-600">
              Tip: Kéo thả để sắp xếp.{" "}
              <span className="font-semibold">Giữ Ctrl</span> khi thả để
              đưa mục vào làm con.
            </div>
          )}
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">
                  (Bước sau mình sẽ thay textarea này bằng TipTap editor đúng
                  chuẩn.)
                </div>
              </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="space-y-2">
                      {assignments.map((a) => {
                        ...
                        const label =
                          prof?.name || prof?.email || a.user_id;
