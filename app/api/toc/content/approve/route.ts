...
export async function POST(req: NextRequest) {
  ...
  // 1) Lấy trạng thái hiện tại + kiểm tra user có phải editor trong sách này không
  ...
  // Kiểm tra role editor ở cấp sách
  ...
  if (pErr || perm?.role !== "editor") {
    return NextResponse.json(
      { error: "Chỉ editor mới được duyệt nội dung" },
      { status: 403 }
    );
  }

  // Nếu version đã published thì thôi
  if (version.status === "published") {
    return NextResponse.json(
      { error: "Phiên bản sách đã được publish, không thể duyệt thêm" },
      { status: 400 }
    );
  }
  ...
  if (!content) {
    return NextResponse.json(
      { error: "Chưa có nội dung để duyệt" },
      { status: 404 }
    );
  }
  ...
  if (content.status !== "submitted") {
    return NextResponse.json(
      {
        error: "Chỉ được duyệt nội dung ở trạng thái 'submitted'",
